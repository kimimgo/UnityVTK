...
public class VtkToUnity
{
	...
	void PolyDataToMesh()
	{
		...
		// Points / Vertices
		int numVertices = (int)pd.GetNumberOfPoints();
		...
		{
			int numCoords = (int)vtkTexCoords.GetNumberOfTuples();
		...
		// Triangles / Cells
		int numTriangles = (int)pd.GetNumberOfPolys();
		...
		if (lines.GetNumberOfCells() > 0)
		{
            ...
            //{
            //    idArray[j] = int.Parse(i.ToString());
            //    j++;
            //}
            //mesh.SetIndices(idArray, MeshTopology.Points, 0);
		...
		int numPointCells = (int)points.GetNumberOfCells();
        if (numPointCells > 0)
        {
            ArrayList idList = new ArrayList();
            Kitware.VTK.vtkIdList pts = Kitware.VTK.vtkIdList.New();
